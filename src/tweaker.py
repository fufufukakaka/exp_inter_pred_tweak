...
class Tweak():
    def search_path(self,estimator, class_labels, aim_label):
        """
    ...
    def esatisfactory_instance(self,x, epsilon, path_info):
        """
    ...
    def feature_tweaking(self,ensemble_classifier, x, class_labels, aim_label, epsilon, cost_func):
        """
        ...
        for estimator in ensemble_classifier:
            if (ensemble_classifier.predict(x.reshape(1, -1)) == estimator.predict(x.reshape(1, -1))
                ...
                paths_info = self.search_path(estimator, class_labels, aim_label)
                for key in paths_info:
                    """ generate epsilon-satisfactory instance """
                    path_info = paths_info[key]
                    es_instance = self.esatisfactory_instance(x, epsilon, path_info)
